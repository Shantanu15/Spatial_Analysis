...
server <- function(input,output){
  ...
  crisil_data <- read.csv("E:\\Google Drive\\R\\Maps\\Data\\Crisil_2016.csv", stringsAsFactors = FALSE)
  out_data$out_per_SHG <- round((out_data$Out_amt/out_data$Out_num_ac)*100000,1)
  mpr_data <- read.csv("E:\\Google Drive\\R\\Maps\\Data\\MPR_Data_Agg.csv", header = TRUE,sep = ",", stringsAsFactors = FALSE)
  ...
  observe({
    ...
    output$Plot_1 <- renderPlotly({ ggplotly(ggplot(data_1, aes(x=long, y= lat,label= DISTRICT, group= group)) + 
                                             ...
                                             +coord_equal() + scale_fill_gradient(low = "#ffffcc", high = "orange", space = "Lab", na.value = "grey", guide = "colourbar"))
    })
